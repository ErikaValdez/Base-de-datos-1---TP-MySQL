...
DROP procedure IF EXISTS `cargar_pedido3`;

DELIMITER $$
USE `terminal_automotriz`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `cargar_pedido3`(IN _id_pedido int,out _mensaje varchar(60))
BEGIN
...
call abm_pedido (789,127,CURDATE(),NULL,'alta',@_respuesta); -- pedido_id , id_concecionaria, fecha_pedido, fecha_entrega
select @_respuesta;
Insert into pedido_del_modelo values (789,1,3); -- Inserta en la tabla pedido_del_modelo :  id_pedido , modelo_id , cantidad;
call cargar_pedido3(789,@_mensaje);  -- Procedimiento 8.
Select @_mensaje; 